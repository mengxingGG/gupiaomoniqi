...
const sqlite3 = require('better-sqlite3')(dbPath);
...
while (count < aiNames.length) {
  ...
  for (let i = 0; i < batchSize; i++) {
    ...
    const cash = (1000000 + Math.random() * 99000000).toFixed(2);
    sqlite3.prepare('INSERT OR IGNORE INTO ai_traders (id, name, psychology_type, strategy_type, cash, total_assets) VALUES (?, ?, ?, ?, ?, ?)').run([id, item.aiName, profile.psychology, profile.strategy, cash, cash]);
  }
  count += batchSize;
  console.log(`Seeded ${count}/${aiNames.length}`);
  if (count < aiNames.length) {
    const start = Date.now();
    while (Date.now() - start < 5000) {}  // 5s pause
  }
}

sqlite3.close();
console.log('✅ 5000 AI traders seeded!');
