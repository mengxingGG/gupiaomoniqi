...
while (count < aiNames.length) {
  ...
  for (let i = 0; i < batchSize; i++) {
    ...
    db.run('INSERT OR IGNORE INTO ai_traders (id, name, psychology_type, strategy_type, cash, total_assets) VALUES (?, ?, ?, ?, ?, ?)', [id, item.aiName, profile.psychology, profile.strategy, cash, cash]);
  }
  count += batchSize;
